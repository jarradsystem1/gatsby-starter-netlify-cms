...
const JoinOurTeam = ({ headingStyle, textStyle, buttonStyle }) => {
  ...
  const strapiHiring = joinTeamQueryData.allStrapiHiring.nodes[0];
  return (
    <JoinSectionWrap>
      <div className='row'>
        <div className='col-2 offset-1'>
          <SectionInner>
            <SectionTitleWrap>
              {title && (
                <Heading {...headingStyle}>{strapiHiring.Header}</Heading>
              )}
              {desc && <Text {...textStyle}>{strapiHiring.Description}</Text>}
