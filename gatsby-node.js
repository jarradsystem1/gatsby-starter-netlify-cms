...
exports.createPages = async ({ graphql, actions }) => {
  ...
  const singleBlog = path.resolve(
    './src/templates/blog-template/blog-template.js'
  );
  const blogList = path.resolve('./src/templates/blog-list/blog-list.js');
  const singleProject = path.resolve(
    './src/templates/project-template/project-template.js'
  );
  ...
  posts.forEach(node => {
    createPage({
      path: node.Title.toLowerCase().replace(/\s/g, '-'),
